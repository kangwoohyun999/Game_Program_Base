...
public class Gun : MonoBehaviour {
    ...
    public bool bBurst { get; set; } = false; // 3점사 on/off
    ...
    private IEnumerator BurstShotRoutine()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            // 사격 불가능한 상태(탄창이 비었거나 재장전 중)라면 점사 중단
            if (state == State.Empty || state == State.Reloading)
            {
                break;
            }

            // 실제 발사 처리 실행 (레이캐스트, 이펙트 등 기존 메서드 활용)
            Shot();

            // 3발 중 마지막 발사가 아니라면, 발사 사이의 미세한 대기 시간 삽입
            if (i < 2)
            {
                yield return new WaitForSeconds(0.06f); // 3점사 사격 간격
            }
        }
    }
    ...
    public void ChangeGun(GunData newGunData)
    {
        ...
        Debug.Log($"무기 교체 완료 {gunData.name}");
    }
}